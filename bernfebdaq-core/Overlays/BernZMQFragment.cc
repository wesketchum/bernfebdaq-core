...
void bernfebdaq::BernZMQFragmentMetadata::CheckTimeWindow() const {
...
std::ostream & bernfebdaq::operator << (std::ostream & os, BernZMQFragmentMetadata const& m){
...
std::ostream & bernfebdaq::operator << (std::ostream & os, BernZMQEvent const & e){
  os << "\nBernZMQEvent"
...
std::string bernfebdaq::BernZMQEvent::hdr_str() const
{
  std::stringstream ss;
  ss << std::hex << "0x" << MAC5() << " " << std::dec
     << flags << "  "
     << Time_TS0() << " (" << IsOverflow_TS0() << "," << IsReference_TS0() << ")  "
     << Time_TS1() << " (" << IsOverflow_TS1() << "," << IsReference_TS1() << ")  ";
  return ss.str();
}

std::ostream & bernfebdaq::operator << (std::ostream & os, BernZMQFragment const & f) {
  os << "BernZMQFragment: "
     << "\n" << *(f.metadata());
...
bool bernfebdaq::BernZMQFragment::Verify() const {
  bool verified=true;

  if(metadata()->n_events()*sizeof(BernZMQEvent) != DataPayloadSize() )
    verified = false;
